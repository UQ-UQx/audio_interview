(function(global) {
  ...
  let audio_context, source;

  let __log = function(e, data) {
  ...
  let Recorder = function(cfg) {
    initWorker();
    let config = cfg || {};
    let bufferLen = config.bufferLen || 4096;
    let self = this;
    let btnPlay = document.createElement('button');
    let btnRecord = document.createElement('button');
      ...
      fileReader.onload = function() {
        ...
        encoderWorker.onmessage = function(e) {
          if (e.data.cmd == 'data') {
            ...
            let mp3Blob = new Blob([new Uint8Array(e.data.buf)], {
              type: 'audio/mp3',
            });
  ...
  var startUserMedia = function(stream) {
    ...
    let recorders = document.querySelectorAll('.RecordMP3js-recorder');
