...
class Lti
{
    ...
    function __construct($config, $display_errors = false)
    {
        ...
        if (!$this->testing || isset($this->ltivars["oauth_consumer_key"])) {
            ...
            if (
                isset(
                    $this->config['lti_keys'][
                        $this->ltivars["oauth_consumer_key"]
                    ]
                )
            ) {
                $lti_key = $this->config['lti_keys'][
                    $this->ltivars["oauth_consumer_key"]
                ];
                $store->add_consumer(
                    $this->ltivars["oauth_consumer_key"],
                ...
                try {
                    ...
                    $this->errors =
                        'Bad LTi Validation (possible incorrect secret) - ' .
                        $e->getMessage();
                }
            } else {
    ...
    function usedummydata()
    {
        $this->ltivars = array(
            ...
            'lis_result_sourcedid' =>
                'course-v1%3AUQx%2BCOURSECODEx%2B2T2017:courses.edx.org-aa766098b5a94a738b54e89caf3a8973:f770caedc6d860f087297810891526d7',
